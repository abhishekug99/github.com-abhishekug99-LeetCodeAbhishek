...
#         @lru_cache(None)
#         def dp(j, h_prev, h_prev2):
#             # j = current column to decide
#             if j == n:
#                 # score last column (n-1) using h_prev and dummy -1
#                 limit = max(h_prev2, -1)
#                 return col_sum(n-1, h_prev+1, limit)

#             best = 0
#             for h in range(-1, n):
#                 gain = 0
#                 if j > 0:
#                     # now we can score column j-1
#                     limit = max(h_prev2, h)
#                     gain = col_sum(j-1, h_prev+1, limit)

#                 best = max(best, gain + dp(j+1, h, h_prev))
